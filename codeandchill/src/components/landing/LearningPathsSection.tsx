      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {paths.map((path, index) => (
          ...
          >
            ...
            <h3 className="text-xl font-semibold bg-gradient-to-r from-white via-gray-100 to-gray-200 bg-clip-text text-transparent mb-2">{path.title}</h3>
            <p className="bg-gradient-to-r from-slate-400 via-slate-300 to-slate-400 bg-clip-text text-transparent text-sm mb-4">{path.description}</p>
            <div className="flex items-center gap-2 text-sm">
              <span className="text-purple-400 font-medium">{path.courses} courses</span>
              <span className="text-slate-600">•</span>
              <span className="bg-gradient-to-r from-slate-500 via-slate-400 to-slate-500 bg-clip-text text-transparent">Start learning →</span>
            </div>
          </div>
        ))}
      </div>
