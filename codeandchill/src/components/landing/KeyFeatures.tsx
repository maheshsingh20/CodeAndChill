...
export function KeyFeatures() {
  const features = [
    {
      icon: <Code className="h-8 w-8 text-lime-400" />,
    ...
    {
      icon: <Briefcase className="h-8 w-8 text-lime-400" />,
      title: "Placement-Focused",
      description:
        "Prepare with mock interviews, aptitude tests, and resume help.",
      bg: "bg-gray-800",
    },
    {
    ...
    {
      icon: <Edit className="h-8 w-8 text-lime-400" />,
  ...
  return (
    <section className="py-20 lg:py-28 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature) => (
            <Card
              key={feature.title}
              className={`p-6 rounded-2xl border border-cyan-400 shadow-neon-lg hover:shadow-neon-xl hover:scale-[1.03] transition-all duration-300 flex flex-col items-center text-center bg-gray-800`}
            >
              <div
                className={`p-4 rounded-full mb-4 ${feature.bg} flex items-center justify-center`}
              >
                {feature.icon}
              </div>
              <CardHeader className="p-0">
                <CardTitle className="text-cyan-400 drop-shadow-neon">
                  {feature.title}
                </CardTitle>
              </CardHeader>
              <CardDescription className="mt-2 flex-grow text-cyan-300/90">
                {feature.description}
              </CardDescription>
            </Card>
          ))}
